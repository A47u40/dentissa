...
export default function SpecialistBookAppointment() {
  const { user } = useAuth()
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!form.paciente || !form.fecha_cita || !form.hora_inicio) {
    ...
    try {
      ...
      setForm({ paciente: '', servicio_id: '', fecha_cita: '', hora_inicio: '' })
    } catch (err) {
    ...
    <div className="flex min-h-screen">
      ...
      <main className="page-with-sidebar max-w-2xl">
        <div className="page-header animate-fade-in-down">
          ...
          <p className="page-subtitle">Agenda una cita para uno de tus pacientes.</p>
        </div>
                    ...
                    {citasExistentes
                      ...
                      .map(c => (
                        <p key={c.id} className="text-xs text-yellow-600">
                          • {c.hora_inicio.slice(0,5)}–{c.hora_fin.slice(0,5)} — {c.especialista_nombre} con {c.paciente_nombre}
                        </p>
                      ))}
                  </div>
                </div>
